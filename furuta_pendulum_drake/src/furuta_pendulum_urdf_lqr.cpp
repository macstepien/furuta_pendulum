...
void DoMain()
{
  ...
  cp_context->FixInputPort(furuta_pendulum_actuation_port, Value<systems::BasicVector<double>>(u0));
  ...
  auto lqr = builder.AddSystem(systems::controllers::LinearQuadraticRegulator(
    *cp, *cp_context, Q, R, N, furuta_pendulum_actuation_port));
