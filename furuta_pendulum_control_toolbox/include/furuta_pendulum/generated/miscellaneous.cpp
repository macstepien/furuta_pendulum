    ...
    const HomogeneousTransforms& ht)
{
    ...
    tmpSum += inertiaProps.getMass_arm1() *
            ( iit::rbd::Utils::transform(tmpX, inertiaProps.getCOM_arm1()));
    
    tmpX = tmpX * ht.fr_arm1_X_fr_arm2;
    tmpSum += inertiaProps.getMass_arm2() *
            ( iit::rbd::Utils::transform(tmpX, inertiaProps.getCOM_arm2()));
